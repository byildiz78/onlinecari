...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        // Siparişi kaydet - TransactionScope içinde tek bir sorgu ile
        const result = await saveOrderData(orderData, tenantId, req);
...
// Tüm sipariş verilerini kaydeden fonksiyon
async function saveOrderData(
    ...
    try {
        ...
        // 5. T-SQL prosedürü ile tüm kayıtları tek transaction içinde işle
        const query = `
            ...
            -- OrderKey'in daha önce kullanılıp kullanılmadığını kontrol et
            IF EXISTS (SELECT 1 FROM [${tenantId}].bonus_orderheaders WHERE OrderKey = @Header_OrderKey)
            BEGIN
                SET @OrderExists = 1;
                SET @Message = 'Bu OrderKey ile daha önce kayıt yapılmış: ' + @Header_OrderKey;
                SET @IsSuccess = 0;
            END
            ELSE
            BEGIN
                BEGIN TRY
                    ...
                    COMMIT TRANSACTION;
                    SET @IsSuccess = 1;
                    SET @Message = 'Sipariş başarıyla kaydedildi';
                END TRY
                BEGIN CATCH
        ...
        if (result[0].OrderExists) {
            return {
                ...
                errorCode: 'DUPLICATE_ORDER_KEY'
            };
        }
        
        if (!result[0].IsSuccess) {
            console.error('SQL Error details:', {
                operation: 'saveOrderData',
                tenantId,
                message: result[0].Message,
